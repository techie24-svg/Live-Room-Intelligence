...
async function setupDatabase() {
  ...
  return {
    tables: ['rooms', 'reactions', 'questions', 'summaries'],
    migrations: ['rooms.host_pin', 'questions.user_name', 'reactions.session_id unique vote rule'],
  };
}
...
export async function GET(req) {
  try {
    ...
    await sql`SELECT 1`;
    return NextResponse.json({ ok: true, database: 'connected', message: 'Database connection is working.' });
  } catch (error) {
...
export async function POST(req) {
  try {
    ...
    return NextResponse.json({ ok: true, message: 'Feel Pulse database setup completed.', ...result });
  } catch (error) {
    return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
  }
}
