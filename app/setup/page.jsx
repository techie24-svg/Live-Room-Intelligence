    ...
    <main className="min-h-screen px-6 py-10">
      <div className="mx-auto max-w-3xl">
        <div className="rounded-3xl border border-white/10 bg-white/10 p-8 shadow-glow backdrop-blur">
          <p className="mb-3 text-sm uppercase tracking-[0.3em] text-blue-200">Feel Pulse Setup</p>
          <h1 className="text-4xl font-bold md:text-6xl">Run database setup</h1>
          <p className="mt-5 text-slate-300">
            Use this after deploying or after applying a delta zip. It creates missing tables and safely applies migrations for host PINs, unique mood votes, names, questions, and summaries.
          </p>

          <div className="mt-8 rounded-2xl border border-yellow-300/20 bg-yellow-300/10 p-4 text-sm text-yellow-100">
            Optional security: set <span className="font-mono">SETUP_SECRET</span> in Vercel, then enter the same value below before running setup.
          </div>
          ...
          <input
            ...
            placeholder="Only needed if SETUP_SECRET is configured"
            type="password"
        ...
        {result && (
          <div className="mt-6 rounded-3xl border border-emerald-400/20 bg-emerald-500/10 p-5 text-emerald-100">
            <p className="font-semibold">{result.message || 'Setup status'}</p>
